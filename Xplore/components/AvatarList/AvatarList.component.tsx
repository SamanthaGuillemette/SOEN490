        ...
        <View style={styles.avatarListContainer}>
            {props.users.map((user: AvatarProps, index: number) => {
               if(index == 0) 
                ...
                return (
                    <View key={index} style={[styles.avatar, {translateX: -12*index, zIndex: index}]}>
                        <Avatar name={user.name} size={40} inList={true}/>
                    </View>)
            
            }
            )}
            <RNView style={[styles.plusCircle, {borderColor: backgroundSecondary, backgroundColor: primary}]}>
